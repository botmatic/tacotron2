...
from preprocessor import audio
import os
import numpy as np 
# from hparams import hparams
# from parallel_wavenet_vocoder.util import mulaw_quantize, mulaw, is_mulaw, is_mulaw_quantize


def build_from_path(hparams, input_dirs, n_jobs=12, tqdm=lambda x: x):
	"""
	...
	for input_dir in input_dirs:
		with open(os.path.join(input_dir, 'metadata.csv'), encoding='utf-8') as f:
			for line in f:
				...
				text = parts[2]
				futures.append(executor.submit(partial(_process_utterance, hparams, wav_path, parts[0], text)))
				index += 1

	return [future.result() for future in tqdm(futures) if future.result() is not None]


def _process_utterance(hparams, wav_path, name, text):
	"""
	...
	return (name, text, mel_spectrogram)