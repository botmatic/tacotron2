...
from .audio import get_hop_size, adjust_time_resolution, trim
        ...
        for idx in range(len(batch)):
            ...
            if hparams.upsample_conditional_features:
                ...
                if max_time_steps is not None:
                    ...
                    if len(x) > max_steps:
                        max_time_frames = max_steps // get_hop_size()
                        s = np.random.randint(0, len(c) - max_time_frames)
                        ts = s * get_hop_size()
                        x = x[ts:ts + get_hop_size() * max_time_frames]
                        c = c[s:s + max_time_frames, :]
                        assert_ready_for_upsampling(x, c)
            else:
                x, c = adjust_time_resolution(x, c)
...
def eval_model(global_step, writer, device, student, teacher, y, c, g, input_lengths, eval_dir, ema=None):
    ...
    if c is not None:
        if hparams.upsample_conditional_features:
            c = c[idx, :, :length // get_hop_size()].unsqueeze(0)
        else:
